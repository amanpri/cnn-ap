...
from model.base import Layer, Optimizer
from model.utils import generate_batches, format_time, softmax_accuracy, softmax_cross_entropy
...
class SequentialModel:
    ...
    def train(
        ...
        for epoch in range(epochs):
            ...
            self._train_acc.append(softmax_accuracy(y_hat, y_train, slicing))
            self._train_loss.append(softmax_cross_entropy(y_hat, y_train))

            y_hat = self._forward(x_test, training=False)
            test_acc = softmax_accuracy(y_hat, y_test, slicing)
